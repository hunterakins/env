...
env_inputs = dict(  z_ss=np.array([0, bottom_depth]).tolist(),
                ...
                cb=np.array([[cb]]).tolist(),
                rhob=np.array([[pb]]).tolist(),
                attn=np.array([[ab]]).tolist(),
                rmax=rmax,
                dr=dr,
                dz=dz,
                zmplt=zmax,
...
#write_json(root + '/iso/iso.json', env_inputs)
...
class IsoBuilder:
    ...
    def __call__(self, **kwargs):
        ...
        curr_mod = __import__(mod_name)
        root = os.path.dirname(curr_mod.__file__)
        env_dict = read_json(root + '/iso/iso.json')
